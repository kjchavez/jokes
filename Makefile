...
processed_dataset = data/processed/jokes.tfrecord
...
build_processed: jokes/data/make_dataset.py data/external/joke-dataset-master data/processed/vocab.txt
	python -m jokes.data.make_dataset build_dataset '$(input_filepattern)' $(vocab_filename) $(processed_dataset)

models/baseline/model.ckpt-1000: jokes/models/main.py $(processed_dataset) $(vocab_filename)
	python -m jokes.models.main train $(processed_dataset) $(vocab_filename) --steps=1000

train: jokes/models/main.py $(processed_dataset) $(vocab_filename)
